...
def function_calling_PostProcessor(outdir, rootfileshere, outtreename, intreeename, cat, year, json, saveallbranches, globaltag):
    for afile in rootfileshere:
        ...
        if saveallbranches:
            subprocess.call(["./skimonefile.py", "--cat=%s"%cat, "--year=%s"%year, "--json=%s"%json, "--saveallbranches", "--globaltag=%s"%globaltag, afile, outfname, intreeename, outtreename])
            #subprocess.call(["./skimonefile.py", "--year=%s"%year, "--json=%s"%json, "--saveallbranches", "--globaltag=%s"%globaltag, afile, outfname, intreeename, outtreename])
        else:
            subprocess.call(["./skimonefile.py", "--cat=%s"%cat, "--year=%s"%year, "--json=%s"%json, "--globaltag=%s"%globaltag, afile, outfname, intreeename, outtreename])
    pass


class Nanoaodprocessor:
    def __init__(self, outdir, indir, outtree, intree, cat, year, json, split, batch, skipold, recursive, flat, saveallbranches, globaltag):
        self.outdir = outdir
        self.indir = indir
        ...
        if len(rootfileshere)>0:
            ...
            if self.split>1: # use multiprocessing
                ...
                for i in range(njobs):
                    ...
                    p = Process(target=function_calling_PostProcessor, args=(outputdirectory, filesforjob, self.outtreename, self.intreeename, self.cat, self.year, self.json, self.saveallbranches, self.globaltag)) # positional arguments go into kwargs
                    p.start()
                    ap.append(p)
                for proc in ap:
                    proc.join()
...
def Nanoaodprocessor_singledir(outputroot, indir, outtree, intree, cat, year, json, split, recursive, saveallbranches, globaltag):
    ...
    for afile in rootfilestoprocess:
        t.Add(afile)
    aproc = ROOT.SkimEvents(t, outputroot, cat, year, json, globaltag, split)
    aproc.setupAnalysis()
...
if __name__=='__main__':
    ...
    parser.add_option("-C", "--cat",  dest="cat", type="string", default="", help="ttbar categorization based in the full phase space")
    ...
    if not options.allinone:
        n=Nanoaodprocessor(outdir, indir, outtree, intree, options.cat, options.year, options.json, options.split, options.batch, options.skipold, options.recursive, options.flat, options.saveallbranches, options.globaltag)
        n.process()
    else:
        Nanoaodprocessor_singledir(outdir, indir, outtree, intree, options.cat, options.year, options.json, options.split, options.recursive, options.saveallbranches, options.globaltag) # although it says outdir, it should really be a output ROOT file name
