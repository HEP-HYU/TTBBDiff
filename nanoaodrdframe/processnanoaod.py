...
def function_calling_PostProcessor(outdir, rootfileshere, outtreename, intreeename, year, json, saveallbranches, globaltag):
    for afile in rootfileshere:
        ...
        if saveallbranches:
            subprocess.call(["./processonefile.py","--year=%s"%year, "--json=%s"%json, "--saveallbranches", "--globaltag=%s"%globaltag, afile, outfname, intreeename, outtreename])
        else:
            subprocess.call(["./processonefile.py", "--year=%s"%year, "--json=%s"%json, "--globaltag=%s"%globaltag, afile, outfname, intreeename, outtreename])
    pass


class Nanoaodprocessor:
    def __init__(self, outdir, indir, outtree, intree, year, json, split, skipold, recursive, saveallbranches, globaltag):
        self.outdir = outdir
        self.indir = indir
        ...
        if len(rootfileshere)>0:
            ...
            if self.split>1: # use multiprocessing
                ...
                for i in range(njobs):
                    ...
                    p = Process(target=function_calling_PostProcessor, args=(outputdirectory, filesforjob, self.outtreename, self.intreeename, self.year, self.json, self.saveallbranches, self.globaltag)) # positional arguments go into kwargs
                    p.start()
                    ap.append(p)
                for proc in ap:
                    proc.join()
                ...
                for afile in rootfileshere:
                    ...
                    subprocess.call(["./processonefile.py", "--year=%s"%self.year, "--json=%s"%self.json, "--saveallbranches=%s"%self.saveallbranches, "--globaltag=%s"%self.globaltag, afile, outfname, self.intreeename, self.outtreename])
...
def Nanoaodprocessor_singledir(outputroot, indir, outtree, intree, year, json, split, recursive, saveallbranches, globaltag):
    ...
    for afile in rootfilestoprocess:
        t.Add(afile)
    aproc = ROOT.LQtopAnalyzer(t, outputroot, year, json, globaltag, split)
    aproc.setupAnalysis()
    ...
    for arootfile in rootfilestoprocess:
        ...
        counterhistogram = intf.Get("hnEvents")
...
if __name__=='__main__':
    ...
    if not options.allinone:
        n=Nanoaodprocessor(outdir, indir, outtree, intree, options.year, options.json, options.split, options.skipold, options.recursive, options.saveallbranches, options.globaltag)
        n.process()
    else:
        Nanoaodprocessor_singledir(outdir, indir, outtree, intree, options.year, options.json,  options.split, options.recursive, options.saveallbranches, options.globaltag) # although it says outdir, it should really be a output ROOT file name
